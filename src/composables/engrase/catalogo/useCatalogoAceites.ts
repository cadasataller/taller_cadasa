...
export function useCatalogoAceites() {
  ...
  function updateDraft(value: CatalogoAceiteGuardarInput): void { draft.value = value; store.limpiarErrorGuardado(); if (fieldErrors.value.nombre && value.nombre.trim()) fieldErrors.value = {}; }
  ...
  return { ...state, modo, draft, original, drawerOpen, hasChanges, canSubmit, confirmacionAbierta, confirmarDescarteAbierto, filtrosMobileAbiertos, fieldErrors, successMessage, inicializar: store.inicializar, reintentar: store.reintentar, actualizarBusqueda: store.actualizarBusqueda, actualizarSistema: store.actualizarSistema, actualizarEstado: store.actualizarEstado, actualizarUso: store.actualizarUso, actualizarOrden: store.actualizarOrden, limpiarFiltros: store.limpiarFiltros, abrirCrear, abrirEditar, updateDraft, validateName, solicitarCierre, cerrarAhora, submit, confirmarActualizacion: guardarDirectamente, cancelarConfirmacion, cancelarDescarte };
}
