...
export function useCatalogoSistemas() {
  ...
  function updateDraft(value: CatalogoSistemaGuardarInput): void { if (!canSave.value) return; draft.value = value; store.limpiarErrorGuardado(); if (fieldErrors.value.nombre && value.nombre.trim()) fieldErrors.value = {}; }
  ...
  return { ...state, modo, draft, original, drawerOpen, hasChanges, canSubmit, canSave, canCreateCatalogItems, canEditCatalogItems, confirmacionAbierta, confirmarDescarteAbierto, filtrosMobileAbiertos, fieldErrors, successMessage, inicializar: store.inicializar, reintentar: store.reintentar, actualizarBusqueda: store.actualizarBusqueda, actualizarEstado: store.actualizarEstado, actualizarUso: store.actualizarUso, actualizarOrden: store.actualizarOrden, limpiarFiltros: store.limpiarFiltros, abrirCrear, abrirEditar, updateDraft, validateName, solicitarCierre, cerrarAhora, submit, confirmarActualizacion, cancelarConfirmacion, cancelarDescarte };
}
