...
function updateRange(value: Date | Date[] | null): void {
  ...
  const [startDate, endDate] = parsedRange.data;
...
<template>
  <section
    ...
    class="flex shrink-0 flex-col gap-3 bg-second px-3 py-3 lg:grid lg:grid-cols-[250px_minmax(0,1fr)_250px] lg:items-end lg:gap-3 lg:px-5"
  >
    <div id="equipment-report-date-filter" class="w-full lg:col-start-1">
      <div class="flex h-9 w-full items-center lg:h-8 lg:w-64">
        <VueDatePicker
          class="w-full"
    ...
    <nav
      ...
      class="-mx-3 flex h-10 snap-x snap-mandatory overflow-x-auto border-y border-gray-200 bg-white px-3 lg:col-start-2 lg:mx-0 lg:h-8 lg:shrink-0 lg:overflow-visible lg:border-x lg:px-0"
    ...
    <div
      class="grid grid-cols-1 gap-2 lg:col-start-3 lg:flex lg:justify-self-end"
    >
      <button
        type="button"
