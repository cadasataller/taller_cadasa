...
import type { OperatorEngineUsageRow } from "@/stores/dashboard/reporte-equipos/reporteEquipos.types";
defineProps<{ rows: OperatorEngineUsageRow[]; operatorLabel: string }>();
</script>
<template>
  <article
    id="operator-engine-usage-card"
    class="flex min-h-28 flex-col rounded-[10px] border border-gray-200 bg-white p-2.5 shadow-sm"
  >
    <h3 class="text-xs font-bold text-main">Uso de motor del operador</h3>
        ...
        >
          ...
          <td class="py-1.5">
            <EquipmentSummaryPercentBar
              ...
              :tone="row.engineOn ? 'success' : 'accent'"
            />
          </td>
        </tr>
        <tr v-if="!rows.length">
          <td colspan="3" class="py-3 text-center text-gray-500">
            Sin uso de motor registrado.
          </td>
        </tr>
      </tbody>
    </table>
