...
<template>
  <section class="grid h-auto self-start gap-2 lg:grid-cols-2">
    <article
      id="stops-classification-card"
      class="flex h-auto flex-col rounded-[10px] border border-gray-200 bg-white p-2.5 shadow-sm"
    >
      <h3 class="mb-1.5 text-xs font-bold text-main">
        Paradas por clasificación
      </h3>
      <div v-if="classifications.length" class="flex-none overflow-visible">
    ...
    >
      ...
      <div v-if="origins.length" class="flex-none overflow-visible">
        <table class="w-full table-fixed border-collapse text-[10px]">
          <thead>
            <tr class="border-b border-gray-100 text-left text-gray-500">
