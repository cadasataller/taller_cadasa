...
function esCultivo(nombre:string){return nombre.trim().toLocaleLowerCase()==='cultivo'}
function esZafra(nombre:string){return nombre.trim().toLocaleLowerCase()==='zafra'}
</script>
<template>
  <section class="min-h-0 overflow-auto rounded-lg border border-gray-200 bg-white p-3 shadow-sm">
    ...
    <template v-if="equipo">
      <header class="flex items-start justify-between gap-3"><div><p class="text-xs font-semibold text-main">{{equipo.tipo_equipo}}</p><h2 class="font-mono text-lg font-semibold text-main">{{equipo.codigo}}</h2><p class="flex flex-wrap items-center gap-x-1 text-xs text-gray-500">Modelo: {{equipo.subtipo||'Sin modelo'}} · <template v-if="equipo.etapas.length"><span v-for="etapa in equipo.etapas" :key="etapa.id" class="inline-flex items-center gap-0.5"><Sprout v-if="esCultivo(etapa.nombre)" class="h-3 w-3 text-main" /><Wheat v-else-if="esZafra(etapa.nombre)" class="h-3 w-3 text-accent" />{{etapa.nombre}}</span></template><template v-else>Sin etapa</template></p></div><b class="rounded px-1.5 py-1 text-[10px] font-semibold uppercase" :class="equipo.estado==='activo'?'bg-success-bg text-success':'bg-danger-bg text-danger'">{{equipo.estado}}</b></header>
