...
<template>
  <button
    ...
    :class="selected ? 'border-main/40 bg-main/5 shadow-[inset_3px_0_0_var(--color-main)]' : 'border-gray-200'"
    :aria-pressed="selected"
    @click="emit('select', item)"
  >
    <span class="grid h-9 w-9 shrink-0 place-items-center rounded-md bg-main/7 text-main">
      <Filter class="h-4 w-4" aria-hidden="true" />
    </span>
    <span class="min-w-0 flex-1">
      <span class="flex flex-wrap items-start justify-between gap-2">
        <span class="min-w-0 flex-1 truncate text-sm font-semibold text-main">{{ item.nombre }}</span>
      ...
      <span class="mt-1 block text-xs tabular-nums text-gray-500">
        ...
        <span v-if="item.impacto.totalAsignaciones !== item.impacto.totalEquipos">
          · {{ new Intl.NumberFormat('es').format(item.impacto.totalAsignaciones) }} asignaciones
        </span>
      </span>
    </span>
