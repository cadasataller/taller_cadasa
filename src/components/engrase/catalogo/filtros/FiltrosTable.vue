...
const props = defineProps<{
  ...
  sortKey: CatalogoFiltroSortKey;
  sortDirection: CatalogoSortDirection;
}>();
const emit = defineEmits<{
  select: [item: CatalogoFiltroItem, trigger: HTMLElement];
  sort: [key: CatalogoFiltroSortKey];
}>();
function sortIcon(key: CatalogoFiltroSortKey) {
...
function select(item: CatalogoFiltroItem, event: Event): void {
  ...
  >
    <table class="w-full table-fixed border-collapse text-left text-xs">
      <thead class="h-9 bg-gray-50 text-gray-600">
        <tr>
          <th class="w-12 px-3"><span class="sr-only">Filtro</span></th>
          <th class="w-[24%] px-3" :aria-sort="ariaSort('codigo')">
            <button
              type="button"
              class="inline-flex cursor-pointer items-center gap-1 font-semibold"
              @click="emit('sort', 'codigo')"
            >
              Código<component :is="sortIcon('codigo')" class="h-3.5 w-3.5" />
            </button>
          </th>
          <th class="w-[17%] px-3" :aria-sort="ariaSort('compras')">
            <button
              type="button"
              class="inline-flex cursor-pointer items-center gap-1 font-semibold"
              @click="emit('sort', 'compras')"
            >
              En compras<component
                :is="sortIcon('compras')"
                class="h-3.5 w-3.5"
              />
            </button>
          </th>
          <th class="w-[18%] px-3" :aria-sort="ariaSort('estado')">
            <button
              type="button"
              class="inline-flex cursor-pointer items-center gap-1 font-semibold"
      ...
      <tbody>
        <tr
          ...
          :key="item.id"
          tabindex="0"
          class="h-16 cursor-pointer border-t border-gray-100 text-gray-700 outline-none transition hover:bg-main/5 focus-visible:bg-main/7 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-main"
          :class="
            selectedId === item.id
        ...
        >
          <td class="px-3">
            <span
              class="grid h-8 w-8 place-items-center rounded-md bg-main/7 text-main"
              ><Filter class="h-4 w-4" aria-hidden="true"
            /></span>
          </td>
          <td class="break-words px-3 font-semibold text-main">
            {{ item.codigo }}
          </td>
          <td class="px-3">
            <FiltroComprasBadge :en-compras="item.estaEnListaCompras" />
          </td>
