...
const saveError = computed(() =>
  mensajeCatalogoFiltrosError(catalogo.errorGuardado.value),
);
onMounted(() => void catalogo.inicializar());
function openItem(item: CatalogoFiltroItem, trigger: HTMLElement): void {
...
<template>
  ...
  >
    <div
      ...
      :class="
        catalogo.drawerOpen.value
          ? 'lg:pr-[calc(clamp(340px,30vw,420px)+1rem)]'
          : ''
      "
    >
      <FiltrosToolbar
        :busqueda="catalogo.busqueda.value"
        :tipo-filtro-id="catalogo.tipoFiltroId.value"
        :compras="catalogo.compras.value"
        :estado="catalogo.estado.value"
        :tipos-filtro="catalogo.opcionesTiposFiltro.value"
        :can-clear="catalogo.hayFiltrosActivos.value"
        @update-busqueda="catalogo.actualizarBusqueda"
        @update-tipo-filtro="catalogo.actualizarTipoFiltro"
        @update-compras="catalogo.actualizarCompras"
        @update-estado="catalogo.actualizarEstado"
        @open-filters="catalogo.filtrosMobileAbiertos.value = true"
        @clear="catalogo.limpiarFiltros"
        @create="catalogo.abrirCrear"
      />
      <header class="mt-4 border-t border-gray-200 pt-3">
        <h2
          data-catalogo-filtros-heading
          tabindex="-1"
      ...
      >
        <FiltrosListState
          v-if="listState"
          :kind="listState"
          :message="mensajeCatalogoFiltrosError(catalogo.errorInicial.value)"
        ...
        /><template v-else
          ...
          <p class="mt-3 text-xs tabular-nums text-gray-500">
            Mostrando {{ visibleLabel }} de {{ totalLabel }} filtros
          </p></template
        >
      </div>
    </div>
    <FiltrosMobileFilterSheet
      v-if="catalogo.filtrosMobileAbiertos.value"
      :open="catalogo.filtrosMobileAbiertos.value"
      :tipo-filtro-id="catalogo.tipoFiltroId.value"
      :compras="catalogo.compras.value"
      :estado="catalogo.estado.value"
      :tipos-filtro="catalogo.opcionesTiposFiltro.value"
      :result-count="catalogo.cantidadVisible.value"
      @close="catalogo.filtrosMobileAbiertos.value = false"
      @reset="catalogo.limpiarFiltros"
      @update-tipo-filtro="catalogo.actualizarTipoFiltro"
      @update-compras="catalogo.actualizarCompras"
      @update-estado="catalogo.actualizarEstado"
    />
    <FiltroDetailDrawer
      ...
      @blur-code="catalogo.validateCode"
