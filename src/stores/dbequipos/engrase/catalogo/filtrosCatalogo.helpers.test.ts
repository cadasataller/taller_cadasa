...
describe("helpers del catálogo de filtros",()=>{
  ...
  it("ordena las tres claves disponibles en ambas direcciones",()=>{ expect(ordenarCatalogoFiltros(items,"codigo","asc").map(x=>x.id)).toEqual([3,2,1]); expect(ordenarCatalogoFiltros(items,"codigo","desc").map(x=>x.id)).toEqual([1,2,3]); expect(ordenarCatalogoFiltros(items,"compras","asc")[0]?.id).toBe(2); expect(ordenarCatalogoFiltros(items,"estado","asc")[0]?.id).toBe(2); });
